...
function scriptRunBody(args: Record<string, any>): Record<string, unknown> {
  const body: Record<string, unknown> = { runAs: args.runAs ?? "SYSTEM", parameters: args.parameters ?? {} };
  ...
  switch (name) {
    ...
    case "run_script_and_wait": {
      ...
      while (true) {
        ...
        const activitiesResp = await ninjaGet(env, `/device/${args.device_id}/activities`, uid ? { pageSize: "25" } : { activityType: "SCRIPT", pageSize: "10" }) as any;
        const list0: any[] = activitiesResp.activities || (Array.isArray(activitiesResp) ? activitiesResp : []);
        const list: any[] = uid ? list0.filter((a) => String(a.sourceConfigUid ?? "") === uid && String(a.statusCode ?? "") === "COMPLETED") : list0;
