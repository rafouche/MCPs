...
// POST /device/{id}/script/run body: a library script by id, or an
// "Install Application" / built-in automation by uid (type ACTION). NinjaOne's
// /automation/scripts listing does not include installer automations, so
// their uid comes from the caller (the remediation whitelist entry) - it is
// the sourceConfigUid on the ACTION activities such a run produces.
function scriptRunBody(args: Record<string, any>): Record<string, unknown> {
  // NinjaOne's `parameters` is a STRING (the script's command-line/preset
  // parameters), not a map - an object gets 400 "Cannot deserialize value of
  // type java.lang.String from Object value" (first real run, 2026-09-22).
  // Accept a string as-is, turn a map into "key=value" pairs, omit when empty.
  // runAs is NinjaOne's own identifier: "system" (lowercase) is the built-in
  // SYSTEM account; anything else is looked up as a stored credential name,
  // and "SYSTEM" in caps failed a real run with "Unable to retrieved
  // credential information" / "Node credential access denied" (2026-09-22).
  const runAsRaw = String(args.runAs ?? "system");
  const body: Record<string, unknown> = { runAs: /^system$/i.test(runAsRaw) ? "system" : runAsRaw };
  const p = args.parameters;
  if (typeof p === "string" && p.trim()) body.parameters = p.trim();
  else if (p && typeof p === "object" && Object.keys(p).length) body.parameters = Object.entries(p).map(([k, v]) => `${k}=${v}`).join(" ");
  if (args.script_uid) { body.type = "ACTION"; body.uid = String(args.script_uid); }
  else if (args.script_id !== undefined && args.script_id !== null) { body.id = Number(args.script_id); }
  else throw new Error("script_id or script_uid is required.");
  return body;
}
// Verified 2026-09-21: a client-credentials API key CAN run library scripts
// by id but NinjaOne refuses type ACTION (installer automations by uid) with
// 403 user_context_required - those need a user-context (authorization-code)
// token this Worker does not hold. Say so plainly instead of a bare 403.
async function runScriptPost(env: Env, args: Record<string, any>): Promise<unknown> {
  try {
...
const TOOLS = [
  ...
  { name: "run_script_on_device", description: "Run an automation script on a device by script ID. IMPORTANT: this only QUEUES the script - POST /device/{id}/script/run is fire-and-forget, the device picks it up on its next check-in and this call's response never contains the script's actual output, no matter how long you wait after calling it. For a script whose result you need to read back in the same investigation (e.g. a diagnostic that writes JSON to the device's activity log), use run_script_and_wait instead - it queues the same way but polls for you and returns the real result (or an honest 'not done yet' signal) in one call.", inputSchema: { type: "object", properties: { device_id: { type: "number" }, script_id: { type: "number", description: "Script ID from list_automation_scripts" }, script_uid: { type: "string", description: "Instead of script_id: the automation's uid (GUID), for 'Install Application' automations, which NinjaOne's API does not list under list_automation_scripts. The remediation whitelist entry carries it as script_uid." }, runAs: { type: "string", description: "'system' (default, the built-in SYSTEM account) or a NinjaOne credential name / role id (e.g. LOGGED_IN_USER) exactly as NinjaOne knows it" }, parameters: { type: "object", description: "Script parameter key-value pairs" } }, required: ["device_id"] } },
  { name: "run_script_and_wait", description: "Run a script on a device and poll for its completed result, for a diagnostic script whose output you actually need to read (e.g. a speed test that writes its result to the device's activity log) - not for a fix-and-move-on script where nothing needs reading back (use run_script_on_device for those; polling adds real wall-clock delay for no benefit there). Queues via the same POST /device/{id}/script/run as run_script_on_device, then polls GET /device/{id}/activities (activityType SCRIPT) every 5 seconds, looking for a new activity timestamped after the script was queued, until maxWaitSeconds elapses. Returns {completed: true, activity: {...}} with the full matched activity object (read whatever fields it actually contains - NinjaOne's own SCRIPT activity schema, e.g. status/activityResult/data, isn't fully pinned down here, so no field is silently dropped or over-parsed) if found in time, or {completed: false, message: ...} if not - a device that's offline or on a slow check-in interval can legitimately take longer than any reasonable wait budget, and that's a real, expected outcome to report honestly, not an error to retry blindly. Cloudflare's own request wall-clock limit is an unverified assumption here (not confirmed against this deployment's actual plan) - if calls consistently time out or error before maxWaitSeconds is reached, that's this limit being hit in practice, not a bug in the polling logic; lower maxWaitSeconds or fall back to run_script_on_device + a later separate list_device_activities check instead.", inputSchema: { type: "object", properties: { device_id: { type: "number" }, script_id: { type: "number", description: "Script ID from list_automation_scripts" }, script_uid: { type: "string", description: "Instead of script_id: the automation's uid (GUID) for an 'Install Application' automation (not listed by list_automation_scripts; the whitelist entry carries it as script_uid). The wait then matches the ACTION activity whose sourceConfigUid equals this uid." }, runAs: { type: "string", description: "'system' (default, the built-in SYSTEM account) or a NinjaOne credential name / role id (e.g. LOGGED_IN_USER) exactly as NinjaOne knows it" }, parameters: { type: "object", description: "Script parameter key-value pairs" }, maxWaitSeconds: { type: "number", description: "How long to poll before giving up and returning completed:false (default 60, capped at 120 - kept conservative pending real confirmation of how long a single call here can safely run)." } }, required: ["device_id"] } },
...
async function runTool(name: string, args: Record<string, unknown>, env: Env): Promise<string> {
  switch (name) {
    ...
    case "run_script_on_device": return JSON.stringify(await runScriptPost(env, args), null, 2);
    case "run_script_and_wait": {
      const queuedAtSec = Math.floor(Date.now() / 1000);
      const uid = args.script_uid ? String(args.script_uid) : null;
      await runScriptPost(env, args);
      ...
      await sleep(2000);
      while (true) {
        // An installer automation (run by uid) shows up as ACTION activities
        // (sourceConfigUid = the uid; STARTED then COMPLETED with activityResult),
        // not SCRIPT ones - so poll unfiltered and match on the uid + COMPLETED.
        // A script run is logged as ACTIONSET START_REQUESTED then ACTION
        // STARTED / COMPLETED (activityResult SUCCESS|FAILURE) - there is no
        // "SCRIPT" activityType, and asking for one 500s (InvalidFilterException,
        // first real run 2026-09-22). Poll unfiltered and match the COMPLETED
        // ACTION after the queue time (by sourceConfigUid when run by uid).
        const activitiesResp = await ninjaGet(env, `/device/${args.device_id}/activities`, { pageSize: "25" }) as any;
        const list0: any[] = activitiesResp.activities || (Array.isArray(activitiesResp) ? activitiesResp : []);
        const list: any[] = list0.filter((a) => String(a.activityType ?? "") === "ACTION" && String(a.statusCode ?? "") === "COMPLETED" && (!uid || String(a.sourceConfigUid ?? "") === uid));
    ...
    case "ninja_api_get": {
      ...
      if (!path.startsWith("/") || path.includes("..")) throw new Error("ninja_api_get: path must start with '/' and contain no '..'.");
      ...
      const text = JSON.stringify(await ninjaGet(env, path, params), null, 2);
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // The NinjaOne sign-in routes are hit by a technician's browser, which
    // cannot send the token; /oauth/callback is protected by its own
    // one-time state cookie, /oauth/start only redirects to NinjaOne's login,
    // and /oauth/status reports whether a sign-in is stored (no secrets).
    const publicRead = (request.method === "GET" || request.method === "HEAD") && ["/health", "/status", "/licenses", "/oauth/start", "/oauth/callback", "/oauth/status"].includes(url.pathname);
