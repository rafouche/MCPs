export interface Env {
  ...
  // agent's Halo email replaces ON_CALL_EMAIL, and their text address is the
  // Mobile Number on their Halo agent record (the `sms` field, digits only)
  // at the email-to-SMS service domain ON_CALL_SMS_DOMAIN (default
  // altec.text.email): <mobile>@altec.text.email. No mobile number on the
  // record = email only. Nobody scheduled, or the lookup fails: ON_CALL_EMAIL
  // / ON_CALL_CC_EMAILS are the fallback, and the response and audit note say
  // which was used. ON_CALL_LOOKUP "off" restores fixed recipients only.
...
const TOOLS = [
  ...
  { name: "get_on_call", description: "Who is on call right now (or at `at`), read from Halo's own Shifts calendar: the agent whose On-call shift covers that moment, with the email (Halo agent record) and text address (the record's Mobile Number at the email-to-SMS service domain; none = email only) escalate_emergency would page, or the fixed fallback recipients when nobody is scheduled. Read-only; use it to check the schedule before relying on it.", inputSchema: { type: "object", properties: { at: { type: "string", description: "Optional ISO timestamp (UTC, e.g. 2026-09-22T01:45:00Z) to evaluate instead of now" } }, required: [] } },
...
async function runTool(name: string, args: Record<string, unknown>, env: Env): Promise<string> {
  switch (name) {
    ...
    case "escalate_emergency": {
      ...
      const onCallWho = onCallResolved ? { source: onCallResolved.source, at: onCallResolved.at, agents: onCallResolved.on_call.map((p) => ({ agent_id: p.agent_id, name: p.agent_name, email: p.email, mobile: p.mobile, sms: p.sms, shift: p.shift })), fallback: onCallResolved.fallback, lookup_error: onCallResolved.error ?? null } : null;
    ...
    case "get_on_call": {
      ...
      return JSON.stringify({ at: r.at, lookup: r.lookup, source: r.source, would_page: { emailto: r.emailto || null, emailcc: r.emailcc }, on_call: r.on_call, fallback: r.fallback, shift_type_id: Number(env.ON_CALL_SHIFT_TYPE_ID || 1), sms_domain: (env.ON_CALL_SMS_DOMAIN || "altec.text.email").replace(/^@/, ""), error: r.error ?? null, note: r.source === "halo" ? (r.on_call.some((p) => p.sms.length) ? "The scheduled on-call agent from Halo's Shifts calendar - email and text." : "The scheduled on-call agent from Halo's Shifts calendar - EMAIL ONLY: no Mobile Number on their Halo agent record.") : r.lookup === "off" ? "ON_CALL_LOOKUP is off - fixed recipients only." : "Nobody has an On-call shift covering this moment in Halo; escalate_emergency would page the fixed fallback recipients." }, null, 2);
    }
    case "halo_api_get": {
