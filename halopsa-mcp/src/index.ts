...
async function runTool(name: string, args: Record<string, unknown>, env: Env): Promise<string> {
  switch (name) {
    ...
    case "escalate_emergency": {
      ...
      const ticketId = isPageTest ? 0 : Number(args.ticket_id);
      if (!isPageTest && (!Number.isInteger(ticketId) || ticketId <= 0)) throw new Error("escalate_emergency: ticket_id must be a positive integer.");
      ...
      const ticket = isPageTest ? { user_name: "", emailtolist: null, client_name: "TEST" } : (await haloGet(env, `/Tickets/${ticketId}`)) as any;
      ...
      const onCallMode = (env.ON_CALL_MODE || "halo").trim().toLowerCase();
      const haloPage = {
      ...
      const pageBody = [isPageTest ? `TEST PAGE - no action needed` : `EMERGENCY - ${clientName}`, isPageTest ? `` : `Help Desk ticket #${ticketId}`, ``, summary, details ? `` : ``, details ? `Found so far: ${details}` : ``, ``, `Sent automatically by Allie. The client has been emailed a brief acknowledgment saying on-call is being notified. This alert ticket exists only to deliver this page - close it once seen.`].filter((l, i, a) => !(l === "" && a[i - 1] === "")).join("\n");
      if (args.dry_run === true) {
        return JSON.stringify({ dry_run: true, would_email_contact: ticket.emailtolist ?? null, acknowledgment: ack, on_call_mode: onCallMode, on_call_alert: onCallMode === "m365" ? { url: alertUrl || null, args: alertArgs } : { halo_alert_ticket: { ...haloPage, summary: pageSubject, body: pageBody, configured: haloPageConfigured } }, status_change: { status_id: args.follow_up_status_id ?? null, agent_id: args.agent_id ?? null, team_id: args.team_id ?? null } }, null, 2);
      }
      ...
      let onCall: Record<string, unknown> = { sent: false, error: onCallMode === "m365" ? "ON_CALL_ALERT_URL is not configured on this Worker" : "ON_CALL_CLIENT_ID / ON_CALL_USER_ID / ON_CALL_TEAM_ID are not configured on this Worker" };
      if (onCallMode !== "m365" && haloPageConfigured) {
        try {
