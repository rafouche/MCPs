...
async function runTool(name: string, args: Record<string, unknown>, env: Env): Promise<string> {
  switch (name) {
    ...
    case "escalate_emergency": {
      ...
      const pageBody = [`EMERGENCY - ${clientName}`, `Help Desk ticket #${ticketId}`, ``, summary, details ? `` : ``, details ? `Found so far: ${details}` : ``, ``, `Sent automatically by Allie. The client has been emailed a brief acknowledgment saying on-call is being notified. This alert ticket exists only to deliver this page - close it once seen.`].filter((l, i, a) => !(l === "" && a[i - 1] === "")).join("\n");
