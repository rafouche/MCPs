export interface Env {
  ...
  // escalate_emergency (HelpDeskAgent v2.13.0): where to page on-call - the
  // m365-mcp Worker's /mcp endpoint (its send_on_call_alert tool holds the
  // recipients) and, if that Worker has MCP_AUTH_TOKEN set, the bearer to use.
  ON_CALL_ALERT_URL?: string;
  ON_CALL_ALERT_TOKEN?: string;
  EMERGENCY_ACK_SIGNATURE?: string; // overrides the default Allie sign-off below
  // Default paging path (ON_CALL_MODE "halo", wrangler vars): Halo itself
  // sends the page - an internal alert ticket is created under our own client
  // for the on-call contact, in a team the Help Desk pipeline never reads,
  // and an emailed action on it goes to that contact plus ON_CALL_CC_EMAILS
  // (the email-to-SMS gateway). No Graph, no extra credentials; Halo's mail
  // is the same path every client reply already uses. ON_CALL_MODE "m365"
  // uses the m365-mcp Worker via ON_CALL_ALERT_URL instead.
  // ON_CALL_MODE "ticket" (default): the page is a HIDDEN emailed action on the
  // original ticket itself, addressed (emailto/emailcc overrides) to the
  // on-call email + SMS gateway - one email, one text, subject carries the
  // real ticket, nothing the client can see, no second ticket. "halo": an
  // internal alert ticket (older design). "m365": m365-mcp send_on_call_alert.
  ON_CALL_EMAIL?: string;
  ON_CALL_MODE?: string;
  ON_CALL_CLIENT_ID?: string;
  ON_CALL_SITE_ID?: string;
  ON_CALL_USER_ID?: string;
  ON_CALL_TEAM_ID?: string;
  ON_CALL_CC_EMAILS?: string; // optional inbound bearer token - see the check at the top of fetch()
  // Who is on call RIGHT NOW comes from Halo's own Shifts calendar (an
  // appointment of type 4 whose shift_type_id is the "On-call" shift type,
  // default id 1) when ON_CALL_LOOKUP is "halo" (the default). The scheduled
  // agent's Halo email replaces ON_CALL_EMAIL and their text address comes
  // from ON_CALL_SMS_MAP (JSON: agent id or email -> gateway address, e.g.
  // {"28":"4178300075@vtext.com"}). Nobody scheduled, or the lookup fails:
  // ON_CALL_EMAIL / ON_CALL_CC_EMAILS are the fallback, and the response and
  // audit note say which was used. ON_CALL_LOOKUP "off" restores fixed
  // recipients only.
  ON_CALL_LOOKUP?: string;
  ON_CALL_SHIFT_TYPE_ID?: string;
  ON_CALL_SMS_MAP?: string;
...
type OnCallPerson = { agent_id: number; agent_name: string; email: string | null; sms: string[]; shift: { appointment_id: number; subject: string; start: string; end: string; shift_type_id: number } };
type OnCallResolution = { at: string; lookup: "halo" | "off"; source: "halo" | "fallback" | "none"; on_call: OnCallPerson[]; fallback: { email: string; cc: string[] }; emailto: string; emailcc: string[]; error?: string };
function splitAddresses(raw: string | undefined): string[] {
  return (raw || "").split(/[;,]/).map((x) => x.trim()).filter(Boolean);
}
function smsFor(env: Env, agentId: number, email: string | null): string[] {
  const raw = (env.ON_CALL_SMS_MAP || "").trim();
  if (!raw) return [];
  let map: Record<string, unknown>;
  try { map = JSON.parse(raw); } catch { return []; }
  const hit = map[String(agentId)] ?? (email ? map[email.toLowerCase()] ?? map[email] : undefined);
  if (Array.isArray(hit)) return hit.map(String).map((x) => x.trim()).filter(Boolean);
  return typeof hit === "string" ? splitAddresses(hit) : [];
}
...
// UTC without a suffix. Nothing scheduled, or any error, falls back to the
// fixed ON_CALL_EMAIL / ON_CALL_CC_EMAILS so an emergency is never left unpaged.
async function resolveOnCall(env: Env, at: Date): Promise<OnCallResolution> {
  const lookup = (env.ON_CALL_LOOKUP || "halo").trim().toLowerCase() === "off" ? "off" : "halo";
  const fallback = { email: (env.ON_CALL_EMAIL || "").trim(), cc: splitAddresses(env.ON_CALL_CC_EMAILS) };
  const base: OnCallResolution = { at: at.toISOString(), lookup, source: fallback.email ? "fallback" : "none", on_call: [], fallback, emailto: fallback.email, emailcc: fallback.cc };
  if (lookup === "off") return base;
  try {
    ...
    for (const r of active) {
      ...
      try { const a = (await haloGet(env, `/Agent/${agentId}`)) as any; email = String(a?.email ?? "").trim().toLowerCase() || null; } catch { email = null; }
      people.push({ agent_id: agentId, agent_name: String(r.agent_name ?? ""), email, sms: smsFor(env, agentId, email), shift: { appointment_id: Number(r.id), subject: String(r.subject ?? ""), start: String(r.start_date), end: String(r.end_date), shift_type_id: Number(r.shift_type_id) } });
    }
    const withEmail = people.filter((p) => p.email);
    if (withEmail.length === 0) {
      return { ...base, on_call: people, error: people.length ? `on-call agent(s) ${people.map((p) => p.agent_name || p.agent_id).join(", ")} have no email on their Halo agent record - fell back to fixed recipients` : undefined };
    }
    ...
    return { ...base, source: "halo", on_call: people, emailto, emailcc };
  } catch (err) {
    return { ...base, error: `on-call lookup failed (${(err as Error).message}) - fell back to fixed recipients` };
  }
}
const TOOLS = [
  ...
  { name: "get_on_call", description: "Who is on call right now (or at `at`), read from Halo's own Shifts calendar: the agent whose On-call shift covers that moment, with the email and text address escalate_emergency would page, or the fixed fallback recipients when nobody is scheduled. Read-only; use it to check the schedule before relying on it.", inputSchema: { type: "object", properties: { at: { type: "string", description: "Optional ISO timestamp (UTC, e.g. 2026-09-22T01:45:00Z) to evaluate instead of now" } }, required: [] } },
...
async function runTool(name: string, args: Record<string, unknown>, env: Env): Promise<string> {
  switch (name) {
    ...
    case "escalate_emergency": {
      ...
      const alertUrl = (env.ON_CALL_ALERT_URL || "").trim();
      const details = args.details_for_on_call ? String(args.details_for_on_call).slice(0, 1500) : "";
      const alertArgs = { ticket_id: ticketId, client_name: clientName, issue_summary: summary, details: details || undefined, dry_run: args.dry_run === true };
      const onCallMode = (env.ON_CALL_MODE || "ticket").trim().toLowerCase();
      ...
      const onCallResolved = onCallMode === "ticket" ? await resolveOnCall(env, atArg ?? new Date()) : null;
      const onCallEmail = onCallResolved ? onCallResolved.emailto : (env.ON_CALL_EMAIL || "").trim();
      const haloPage = {
        client_id: Number(env.ON_CALL_CLIENT_ID || 0), site_id: Number(env.ON_CALL_SITE_ID || 0), user_id: Number(env.ON_CALL_USER_ID || 0), team_id: Number(env.ON_CALL_TEAM_ID || 0),
        cc: (onCallResolved ? onCallResolved.emailcc : splitAddresses(env.ON_CALL_CC_EMAILS)).join(";"),
      };
      const haloPageConfigured = haloPage.client_id > 0 && haloPage.user_id > 0 && haloPage.team_id > 0;
      ...
      const ticketPageConfigured = onCallEmail.length > 0;
      const onCallWho = onCallResolved ? { source: onCallResolved.source, at: onCallResolved.at, agents: onCallResolved.on_call.map((p) => ({ agent_id: p.agent_id, name: p.agent_name, email: p.email, sms: p.sms, shift: p.shift })), fallback: onCallResolved.fallback, lookup_error: onCallResolved.error ?? null } : null;
      const onCallLabel = onCallResolved && onCallResolved.source === "halo" ? `scheduled on-call ${onCallResolved.on_call.filter((p) => p.email).map((p) => p.agent_name || `agent ${p.agent_id}`).join(", ")} (Halo shift)` : onCallResolved ? `fixed fallback recipients${onCallResolved.error ? ` - ${onCallResolved.error}` : " - nobody is scheduled on call in Halo"}` : "";
      ...
      if (args.dry_run === true) {
        return JSON.stringify({ dry_run: true, would_email_contact: ticket.emailtolist ?? null, acknowledgment: ack, on_call_mode: onCallMode, on_call_alert: onCallMode === "m365" ? { url: alertUrl || null, args: alertArgs } : onCallMode === "halo" ? { halo_alert_ticket: { ...haloPage, summary: pageSubject, body: pageBody, configured: haloPageConfigured } } : { hidden_action_on_ticket: ticketId, emailto: onCallEmail, emailcc: haloPage.cc, on_call: onCallWho, body: pageBody, configured: ticketPageConfigured }, status_change: { status_id: args.follow_up_status_id ?? null, agent_id: args.agent_id ?? null, team_id: args.team_id ?? null } }, null, 2);
      }
      ...
      // 2. page on-call via the m365-mcp Worker; a failure here never undoes step 1
      let onCall: Record<string, unknown> = { sent: false, error: onCallMode === "m365" ? "ON_CALL_ALERT_URL is not configured on this Worker" : onCallMode === "halo" ? "ON_CALL_CLIENT_ID / ON_CALL_USER_ID / ON_CALL_TEAM_ID are not configured on this Worker" : "ON_CALL_EMAIL is not configured on this Worker" };
      if (onCallMode === "ticket" && ticketPageConfigured) {
        ...
        try {
          ...
          if (haloPage.cc) pagePayload.emailcc = haloPage.cc;
          const posted = (await haloPost(env, "/Actions", [pagePayload])) as any;
          const postedAction = Array.isArray(posted) ? posted[0] : posted;
          onCall = { sent: true, via: "ticket", action_id: postedAction?.id ?? null, to: [onCallEmail].concat(haloPage.cc ? haloPage.cc.split(";") : []), on_call: onCallWho, email_status: postedAction?.email_status ?? null, dateemailed: postedAction?.dateemailed ?? null, emailto_recorded: postedAction?.emailto ?? null, hiddenfromuser: postedAction?.hiddenfromuser ?? null };
        } catch (err) {
          onCall = { sent: false, via: "ticket", on_call: onCallWho, error: (err as Error).message };
        }
      }
      else if (onCallMode === "halo" && haloPageConfigured) {
        try {
          const ticketPayload: Record<string, unknown> = { summary: pageSubject, details: pageBody, client_id: haloPage.client_id, user_id: haloPage.user_id, team_id: haloPage.team_id };
          if (haloPage.site_id > 0) ticketPayload.site_id = haloPage.site_id;
          if (haloPage.cc) ticketPayload.emailcclist = haloPage.cc;
          const createdRaw = (await haloPost(env, "/Tickets", [ticketPayload])) as any;
          const created = Array.isArray(createdRaw) ? createdRaw[0] : createdRaw;
          const alertTicketId = Number(created?.id);
          if (!Number.isInteger(alertTicketId) || alertTicketId <= 0) throw new Error(`alert ticket was not created (response: ${JSON.stringify(createdRaw).slice(0, 300)})`);
          const actionPayload: Record<string, unknown> = { ticket_id: alertTicketId, note: pageBody, note_html: noteToHtml(pageBody), hiddenfromuser: false, outcome_id: 16 };
          if (haloPage.cc) actionPayload.emailcc = haloPage.cc;
          await haloPost(env, "/Actions", [actionPayload]);
          onCall = { sent: true, via: "halo", alert_ticket_id: alertTicketId, to: [created?.user_email ?? created?.emailtolist ?? `contact ${haloPage.user_id}`].concat(haloPage.cc ? haloPage.cc.split(";") : []) };
        } catch (err) {
          onCall = { sent: false, via: "halo", error: (err as Error).message };
        }
      }
      else if (onCallMode === "m365" && alertUrl) {
        try {
          const headers: Record<string, string> = { "Content-Type": "application/json" };
          if (env.ON_CALL_ALERT_TOKEN) headers.Authorization = `Bearer ${env.ON_CALL_ALERT_TOKEN}`;
          const res = await fetch(alertUrl, { method: "POST", headers, body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/call", params: { name: "send_on_call_alert", arguments: alertArgs } }) });
          const rpc = (await res.json()) as any;
          if (!res.ok || rpc?.error) throw new Error(rpc?.error?.message || `HTTP ${res.status}`);
          onCall = JSON.parse(rpc.result.content[0].text);
        } catch (err) {
          onCall = { sent: false, error: (err as Error).message };
        }
      }
      if (isPageTest) return JSON.stringify({ page_test: true, on_call_mode: onCallMode, on_call_alert: onCall }, null, 2);
      // 3. audit note
      const audit = `[EMERGENCY ACK SENT] ${new Date().toISOString()} - acknowledgment emailed to ${ticket.emailtolist ?? "(no address on ticket)"}; on-call ${onCall.sent ? `paged via ${onCall.via ?? "m365"}${onCall.alert_ticket_id ? ` (alert ticket #${onCall.alert_ticket_id})` : ""} to ${(onCall.to as string[] | undefined)?.join(", ") ?? "configured contacts"}${onCallLabel ? ` [${onCallLabel}]` : ""}` : `NOT paged: ${onCall.error}`}. Summary: ${summary}`;
    ...
    case "get_on_call": {
      ...
      return JSON.stringify({ at: r.at, lookup: r.lookup, source: r.source, would_page: { emailto: r.emailto || null, emailcc: r.emailcc }, on_call: r.on_call, fallback: r.fallback, shift_type_id: Number(env.ON_CALL_SHIFT_TYPE_ID || 1), error: r.error ?? null, note: r.source === "halo" ? "The scheduled on-call agent from Halo's Shifts calendar." : r.lookup === "off" ? "ON_CALL_LOOKUP is off - fixed recipients only." : "Nobody has an On-call shift covering this moment in Halo; escalate_emergency would page the fixed fallback recipients." }, null, 2);
    }
    case "halo_api_get": {
