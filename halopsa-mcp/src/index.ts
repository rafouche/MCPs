...
async function runTool(name: string, args: Record<string, unknown>, env: Env): Promise<string> {
  switch (name) {
    ...
    case "escalate_emergency": {
      ...
      const onCallEmail = (env.ON_CALL_EMAIL || "").trim();
      const haloPage = {
        ...
        cc: (env.ON_CALL_CC_EMAILS || "").split(/[;,]/).map((x) => x.trim()).filter(Boolean).join(";"),
      };
      ...
      if (args.dry_run === true) {
        return JSON.stringify({ dry_run: true, would_email_contact: ticket.emailtolist ?? null, acknowledgment: ack, on_call_mode: onCallMode, on_call_alert: onCallMode === "m365" ? { url: alertUrl || null, args: alertArgs } : onCallMode === "halo" ? { halo_alert_ticket: { ...haloPage, summary: pageSubject, body: pageBody, configured: haloPageConfigured } } : { hidden_action_on_ticket: ticketId, emailto: onCallEmail, emailcc: haloPage.cc, body: pageBody, configured: ticketPageConfigured }, status_change: { status_id: args.follow_up_status_id ?? null, agent_id: args.agent_id ?? null, team_id: args.team_id ?? null } }, null, 2);
      }
      ...
      if (onCallMode === "ticket" && ticketPageConfigured) {
        ...
        try {
          ...
          onCall = { sent: true, via: "ticket", action_id: postedAction?.id ?? null, to: [onCallEmail].concat(haloPage.cc ? haloPage.cc.split(";") : []), email_status: postedAction?.email_status ?? null, dateemailed: postedAction?.dateemailed ?? null, emailto_recorded: postedAction?.emailto ?? null, hiddenfromuser: postedAction?.hiddenfromuser ?? null };
        } catch (err) {
          onCall = { sent: false, via: "ticket", error: (err as Error).message };
        }
      }
