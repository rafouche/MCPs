  ...
  switch (name) {
    ...
    case "delete_ticket_note": {
      ...
      if (!hasDraftMarker(note)) {
        throw new Error(`delete_ticket_note: action ${args.action_id}'s note does not contain the literal "[DRAFT PENDING APPROVAL]" marker on its own line - refusing to delete anything that isn't clearly this pipeline's own pending draft.`);
      }
