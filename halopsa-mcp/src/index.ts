...
const TOOLS = [
  ...
  { name: "list_agents", description: "List agents (technicians/staff) in HaloPSA. Excludes inactive/disabled agent accounts by default - pass include_inactive to see those too (e.g. an account whose Halo licence was removed but is still used as an API-only integration identity).", inputSchema: { type: "object", properties: { count: { type: "number" }, include_inactive: { type: "boolean" } } } },
...
async function runTool(name: string, args: Record<string, unknown>, env: Env): Promise<string> {
  switch (name) {
    ...
    case "list_agents": { const p: Record<string, string> = { count: String(args.count ?? 50) }; if (args.include_inactive) p.includeinactive = "true"; return JSON.stringify(await haloGet(env, "/Agent", p), null, 2); }
