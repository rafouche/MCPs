export interface Env {
  ...
  EMERGENCY_ACK_SIGNATURE?: string; // overrides the default Allie sign-off below // optional inbound bearer token - see the check at the top of fetch()
...
async function runTool(name: string, args: Record<string, unknown>, env: Env): Promise<string> {
  switch (name) {
    ...
    case "escalate_emergency": {
      ...
      if (!Number.isInteger(ticketId) || ticketId <= 0) throw new Error("escalate_emergency: ticket_id must be a positive integer.");
      const summary = String(args.issue_summary ?? "").replace(/\s+/g, " ").trim();
      ...
      const clientName = String(args.client_name ?? ticket.client_name ?? "Unknown client").trim();
      const alertUrl = (env.ON_CALL_ALERT_URL || "").trim();
      const alertArgs = { ticket_id: ticketId, client_name: clientName, issue_summary: summary, details: args.details_for_on_call ? String(args.details_for_on_call).slice(0, 1500) : undefined, dry_run: args.dry_run === true };
      if (args.dry_run === true) {
        return JSON.stringify({ dry_run: true, would_email_contact: ticket.emailtolist ?? null, acknowledgment: ack, on_call_alert: { url: alertUrl || null, args: alertArgs }, status_change: { status_id: args.follow_up_status_id ?? null, agent_id: args.agent_id ?? null, team_id: args.team_id ?? null } }, null, 2);
      }
      // 1. the templated acknowledgment, as a real emailed reply (outcome 16)
      const ackResult = await haloPost(env, "/Actions", [{ ticket_id: ticketId, note: ack, note_html: noteToHtml(ack), hiddenfromuser: false, outcome_id: 16 }]);
      // 2. page on-call via the m365-mcp Worker; a failure here never undoes step 1
      let onCall: Record<string, unknown> = { sent: false, error: "ON_CALL_ALERT_URL is not configured on this Worker" };
      if (alertUrl) {
        try {
      ...
      // 3. audit note
      const audit = `[EMERGENCY ACK SENT] ${new Date().toISOString()} - acknowledgment emailed to ${ticket.emailtolist ?? "(no address on ticket)"}; on-call ${onCall.sent ? `paged (${(onCall.to as string[] | undefined)?.join(", ") ?? "configured contacts"})` : `NOT paged: ${onCall.error}`}. Summary: ${summary}`;
