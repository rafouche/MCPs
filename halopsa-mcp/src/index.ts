...
async function runTool(name: string, args: Record<string, unknown>, env: Env): Promise<string> {
  switch (name) {
    ...
    case "mark_draft_approved": {
      ...
      const note: string = typeof action.note === "string" ? action.note : "";
      if (!note.startsWith("[DRAFT PENDING APPROVAL]")) {
        throw new Error(`mark_draft_approved: action ${args.action_id}'s note does not start with the literal "[DRAFT PENDING APPROVAL]" marker - refusing to edit anything that isn't clearly this pipeline's own pending draft.`);
      }
