...
const TOOLS = [
  ...
  { name: "list_time_entries", description: "List time entries (billable/non-billable labor) from HaloPSA tickets", inputSchema: { type: "object", properties: { ticket_id: { type: "number" }, client_id: { type: "number" }, agent_id: { type: "number" }, count: { type: "number" }, start_date: { type: "string" }, end_date: { type: "string" } } } },
  { name: "get_ticket_time_entries", description: "Get all time entries and labor costs for a specific ticket", inputSchema: { type: "object", properties: { ticket_id: { type: "number" } }, required: ["ticket_id"] } },
...
async function runTool(name: string, args: Record<string, unknown>, env: Env): Promise<string> {
  switch (name) {
    ...
    case "list_contacts": { const p: Record<string, string> = { count: String(args.count ?? 50) }; if (args.client_id) p.client_id = String(args.client_id); if (args.search) p.search = String(args.search); return JSON.stringify(await haloGet(env, "/Customer", p), null, 2); }
    case "get_contact": return JSON.stringify(await haloGet(env, `/Customer/${args.contact_id}`), null, 2);
    ...
    case "list_time_entries": { const p: Record<string, string> = { count: String(args.count ?? 50) }; if (args.ticket_id) p.ticket_id = String(args.ticket_id); if (args.client_id) p.client_id = String(args.client_id); if (args.agent_id) p.agent_id = String(args.agent_id); if (args.start_date) p.start_date = String(args.start_date); if (args.end_date) p.end_date = String(args.end_date); return JSON.stringify(await haloGet(env, "/Action", p), null, 2); }
    case "get_ticket_time_entries": return JSON.stringify(await haloGet(env, "/Action", { ticket_id: String(args.ticket_id), count: "100" }), null, 2);
    ...
    case "list_opportunities": { const p: Record<string, string> = { count: String(args.count ?? 20) }; if (args.client_id) p.client_id = String(args.client_id); if (args.search) p.search = String(args.search); return JSON.stringify(await haloGet(env, "/Opportunity", p), null, 2); }
    case "get_opportunity": return JSON.stringify(await haloGet(env, `/Opportunity/${args.opportunity_id}`), null, 2);
    case "list_software_licences": { const p: Record<string, string> = { count: String(args.count ?? 50) }; if (args.client_id) p.client_id = String(args.client_id); if (args.search) p.search = String(args.search); return JSON.stringify(await haloGet(env, "/SoftwareLicences", p), null, 2); }
    case "list_contracts": { const p: Record<string, string> = { count: String(args.count ?? 50) }; if (args.client_id) p.client_id = String(args.client_id); return JSON.stringify(await haloGet(env, "/Contract", p), null, 2); }
    case "get_contract": return JSON.stringify(await haloGet(env, `/Contract/${args.contract_id}`), null, 2);
